...
import { buildingApiClient } from '/src/data/buildingApiClient.js';
import { invalidateCache } from '../../../core/utils/helper/simpleCache.js';
...
/**
 * EditRoomModal
 * A guided, step-by-step wizard for editing an existing flat type — mirrors
 * AddRoomModal's stepper so editing feels exactly like adding, just
 * pre-filled with the flat's current data.
 */
export default function EditRoomModal({ isOpen, onClose, flat, onUpdate }) {
    const { t, lang } = useTranslation();
    const isRTL = lang === 'ar';

    // `flat` can briefly be null/undefined while the modal is closed — all
    // hooks below must still run in the same order every render, so we guard
    // with optional chaining here and only bail out (after every hook has
    // been declared) right before the JSX return.
    ...
    const [existingCover] = useState(flat?.coverimg || '');
    ...
    const validateStep = (index) => {
        ...
        if (key === 'media') {
            const hasCover = !!existingCover || !!coverFile;
            if (!hasCover) e.cover = t('error_cover_required') || 'Cover image is required.';
            const totalImages = existingGalImages.length + galFiles.length;
            if (totalImages === 0) e.gallery = t('error_images_required') || 'Gallery images are required.';
        }
        return e;
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                typeId: Number(formData.unit_breakdown_id) || 0,
            ...
            // 5. Call update API
            const result = await buildingApiClient.putUpdateFlat(payload);

            if (result?.status === true) {
                setStatusMessage({ text: t('flat_updated_success') || 'Flat updated successfully!', isError: false });
                invalidateCache(`flats:${payload.hotelbuildingID}`);
                    ...
                    {current.key === 'review' && (
                        <ReviewStep
                            ...
                            data={{
                                ...
                                mediaCount: (coverFile || existingCover ? 1 : 0) + existingGalImages.length + galFiles.length,
                            }}
                            onEditStep={goToStep}
                        />
                    )}
...
/** ReviewStep — a compact read-only summary of the flat type before saving. */
